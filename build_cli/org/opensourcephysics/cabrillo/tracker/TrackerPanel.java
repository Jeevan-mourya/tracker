...
// static fields
	...
	//An Extension for the 3DTracker Analytical Tool
	private Stereo3DState stereo3DState = null;
	public Stereo3DState getStereo3DState() {
		return stereo3DState;
	}

	public void setStereo3DState(Stereo3DState state) {
		this.stereo3DState = state;
	}
...
//		TrackControl.getControl(trackerPanel).dispose();
	...
	private class TMouseController extends IADMouseController {
		...
		public void mousePressed(MouseEvent e) {
			...
			Video video = getVideo();
			if (video instanceof DualXuggleVideo && getStereo3DState() != null && getStereo3DState().getCalibrationData() != null) {
				...
				if (imgA != null && imgB != null) {
					...
					if (clickX < imgAWidth) {
						getStereo3DState().setLeftClick(new Point2D.Double(clickX, clickY));
					} else {
						getStereo3DState().setRightClick(new Point2D.Double(clickX - imgAWidth, clickY));
					}
					
					if (getStereo3DState().getLeftClick() != null && getStereo3DState().getRightClick() != null) {
						StereoCalibrationManager manager = new StereoCalibrationManager();
						double[] point3D = manager.triangulateCoordinate(
							getStereo3DState().getLeftClick(),
							getStereo3DState().getRightClick(),
							getStereo3DState().getCalibrationData().P1,
							getStereo3DState().getCalibrationData().P2
						);
...
//      control.setValue("version", OSPRuntime.VERSION); //$NON-NLS-1$
			...
			//An Estension for the 3DTracker Analytical Tool
			if (trackerPanel.getStereo3DState()!=null) {
				control.setValue("stereo3d_state", trackerPanel.getStereo3DState()); //$NON-NLS-1$
			}
...
//		addFilter(BarrelPincushionFilter.class);
	...
	protected void reload() {
		...
		for (int i = 0; i < proplist.size(); i++) {
			...
			//An Extension for the 3DTracker Analytical Tool 
			else if (prop.getPropertyName().equals("stereo3d_state")) {
                XMLControl stateControl = prop.getChildControls()[0];
                Stereo3DState loadedState = (Stereo3DState) stateControl.loadObject(null);
                setStereo3DState(loadedState);
            }
		}
		TrackChooserTView.ignoreRefresh = false;
		TFrame.repaintT(this);		
